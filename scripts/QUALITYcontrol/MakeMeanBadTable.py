...
def write_BAD(out_buffer, pd_column, datasets_n, SNP_n):
    out_buffer.write(pack([previous_name, pd_column.mean(), pd_column.median(), datasets_n, SNP_n]))
    ...
    with open(out_path, "w") as out:
        ...
        for file_name in sorted(os.listdir(actual_ploidy_path)):
            ...
            with open(ploidy_path + file_name.replace("_ploidy", "")) as f:
                cur_SNP_number = len(pd.read_table(f).index)
            ...
            if previous_name == cell_line_name:
                ...
                if sum_table is None:
                    ...
                    write_BAD(out, sum_table["BAD"], aligns_number, SNP_number)
                    sum_table = table
                    aligns_number = cur_l
                    SNP_number = cur_SNP_number
                    previous_name = cell_line_name
