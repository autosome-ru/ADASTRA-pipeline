    ...
    for key in cell_lines_dict:
        ...
        for align_path in cell_lines_dict[key]:
            ...
            if out_t is None:
                ...
                out_t = out_t.size().reset_index(name='counts')
                out_t['allele_reads'] = out_t.index
                out_t = out_t.reset_index(drop=True)
                out_t.fillna(0, inplace=True)
            else:
                tmp_df = pd.DataFrame()
                tmp_df['cover'] = (sum_df['ref_read_counts'] + sum_df['alt_read_counts']).value_counts()
                tmp_df['delta'] = (sum_df['ref_read_counts'] - sum_df['alt_read_counts']).value_counts()
