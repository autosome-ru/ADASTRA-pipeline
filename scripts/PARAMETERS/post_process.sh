...
if [ "$indicator" == "--forCL" ];then
  ...
  parallel --jobs "$njobs" python3 "$ScriptsFolder"ASBcalling/post_process.py "CL" :::: "$ParametersListsFolder"/Agr_parameters.cfg
fi
if [ "$indicator" == "--forTF" ];then
  python3 "$ScriptsFolder"PARAMETERS/MakeParametersForAgr.py "TF"
  parallel --jobs "$njobs" python3 "$ScriptsFolder"ASBcalling/post_process.py "TF" :::: "$ParametersListsFolder"/Agr_parameters.cfg
fi