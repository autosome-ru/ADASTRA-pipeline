...
def plot_histogram(n, counts_array, plot_fit=None, save=True):
    ...
    if plot_fit is not None:
        ...
        label = 'negative binom fit for {}\ntotal observations: {}\nr1={:.2f}, r2={:.2f}, p={:.2f}\nq_left={}, q_right={}, w={:.2f}'.format(
            main_allele, total_snps, r1, r2, get_p(), q_left, q_right, w)
...
def fit_negative_binom(counts_array):
    ...
    try:
        x = optimize.minimize(fun=make_log_likelihood(counts_array),
                              x0=np.array([(fix_c, fix_c, 0.5)]),

                              bounds=[(0.00001, None), (0.00001, None), (0, 1)])
    except ValueError:
        return 'NaN'
    return x
...
if __name__ == '__main__':
    ...
    for fix_c in fix_c_array:
        for BAD in [2, 2.5, 4]:
            ...
            q_left = min(x for x in range(number + 1) if cdf(x) >= 0.025)
