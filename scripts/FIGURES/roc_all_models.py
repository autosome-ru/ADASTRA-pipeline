...
labels = dict(zip([1, 4/3, 3/2, 2, 5/2, 3, 4, 5, 6], ['1', '4/3', '3/2', '2', '5/2', '3', '4', '5', '6']))

for sig in ['all', 'all_5', '123456', 'all_but_1.33_2.5']:
    for mult in [2, 3, 4, 5, 6, 7, 8, 9, 10]:
        model = 'CAIC@{}@{:.1f}_tables'.format(sig, mult)
        states = get_states(sig)
        t = {}
        min_tr = {}
        max_tr = {}
        for BAD in states:
            t[BAD] = pd.read_table(os.path.expanduser('~/Desktop/soos_BAD/counts/counts_deltaqm_{}_{:.2f}.tsv'.format(model, BAD)))
        ...
        legend = ax.legend(loc='center left', bbox_to_anchor=(1, 0.5), title='BAD', frameon=True, fancybox=False,
                           labels=[labels[x] for x in states])
        legend.get_frame().set_edgecolor('black')

        # for i, BAD in enumerate(states):
        #     ax.axhline(y=P[BAD] / ALL * 1 / l, color='C'+str(i), linestyle='--', label=None, lw=1)

        plt.savefig(os.path.expanduser('~/Desktop/soos_BAD/PR@{}@{}.png'.format(sig, mult)), dpi=300)
        plt.close(fig)
        ...
        plt.savefig(os.path.expanduser('~/Desktop/soos_BAD/ROC@{}@{}.png'.format(sig, mult)), dpi=300)
