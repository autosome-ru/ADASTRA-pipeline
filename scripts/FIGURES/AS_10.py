...
def local_read_weights():
    ...
    for fixed_allele in ('ref', 'alt'):
        ...
        gof[fixed_allele] = {}
        for BAD in states:
            precalc_params_path = os.path.expanduser('D:\\Sashok\\Desktop/BillCipherStats/NBweights_{}_BAD={:.1f}.npy'.format(fixed_allele, BAD))
...
for BAD in states:
    ...
    t = pd.read_table(os.path.expanduser(
        'D:\\Sashok\\Desktop/BillCipherStats/bias_stats_BAD{:.1f}{}.tsv'.format(BAD,
                                                              {'all': '',
                                                               'K562': '_k562',
                                                               'diploid': '_esc'}[
                                                                  cells])))
    ...
    plt.savefig(os.path.expanduser('D:\\Sashok\\Desktop/BillCipherStats/AS_10/Figure_AS_10_{}_{}_{:.2f}.png'.format(cells, covs[0], BAD)))
    plt.close(fig)

    # gof vs read cov

    df_alt = pd.read_table(os.path.expanduser('D:\\Sashok\\Desktop/BillCipherStats/NBweights_alt_BAD{:.1f}.tsv'.format(BAD)))
    df_ref = pd.read_table(os.path.expanduser('D:\\Sashok\\Desktop/BillCipherStats/NBweights_ref_BAD{:.1f}.tsv'.format(BAD)))
    ...
    plt.savefig(os.path.expanduser('D:\\Sashok\\Desktop/BillCipherStats/AS_10/Figure_AS_10_gof_{:.2f}.png'.format(BAD)))
    plt.close(fig)
    ...
    plt.savefig(os.path.expanduser('D:\\Sashok\\Desktop/BillCipherStats/AS_10/Figure_AS_10_r_scatter_{:.2f}.png'.format(BAD)))
    plt.close(fig)
    ...
    plt.savefig(os.path.expanduser('D:\\Sashok\\Desktop/BillCipherStats/AS_10/Figure_AS_10_r_scatter_ref_alt_{:.2f}.png'.format(BAD)))
    plt.close(fig)
