...
if __name__ == '__main__':
    ...
    # # Barplot
    # df = pd.read_table(os.path.expanduser("D:\\Sashok/Desktop/PARAMETERS/blue_red_stats.tsv"))
    # # df = pd.read_table(os.path.expanduser("D:\\Sashok/Desktop/BillCipherStats/blue_red_stats.tsv"))
    # fig, ax = plt.subplots()
    ...
    # # plt.savefig(os.path.expanduser("D:\\Sashok/Desktop/BillCipherStats/AS_Figure_7_barplot.png"), dpi=300)
    # plt.savefig(os.path.expanduser("D:\\Sashok/Desktop/BillCipherStats/AS_Figure_7_barplot.svg"), dpi=300)
    #
    # plt.close(fig)
    ...
    for name in ['all_tfs.tsv']:  # + top10_names + ['CTCF_for_comparison.tsv']:
        ...
        if name == 'CTCF_for_comparison.tsv':
            mat = True
        pt = pd.read_table(os.path.expanduser("D:\\Sashok/Desktop/BillCipherStats/scatter/{}".format('CTCF_HUMAN.tsv' if mat else name)))
        # pt = pd.read_table(os.path.expanduser("D:\\Sashok/Desktop/Releases/TF_P-values/TF_P-values/{}".format('CTCF_HUMAN.tsv' if mat else name)))
        ...
        xlim = min(max(np.abs(pt['log_pv'])), 310) * 1.05
        ...
        plt.savefig(os.path.expanduser("D:\\Sashok/Desktop/BillCipherStats/AS_7/AS_Figure_7_{}_p_tr={:.2f}_fc_tr={:.2f}_fdr_tr={:.2f}_{}.png".format(
            name.replace('_fc.tsv', ''), perf_tr, fc_tr, fdr_tr, field)), dpi=300)
        # plt.savefig(os.path.expanduser("D:\\Sashok/Desktop/BillCipherStats/AS_Figure_7_{}_p_tr={:.2f}_fc_tr={:.2f}_fdr_tr={:.2f}_{}.png".format(
        #     name.replace('_fc.tsv', ''), perf_tr, fc_tr, fdr_tr, field)), dpi=300)
        plt.close(fig)
