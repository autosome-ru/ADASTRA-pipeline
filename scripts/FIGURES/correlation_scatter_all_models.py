...
for sig in ['123456', 'all_5', 'all']:
    for mult in [2, 3, 4, 5, 6, 7, 8, 9, 10]:
        ...
        all_dsts.append(np.quantile(df[(df[model] != 0) & (df[model] != 0.01)][model], 0.75))
        #
        # ax.grid(True)
        # ax.legend()
...
df['Mean_rank'] = df[['K562_Q3', 'MCF7_Q3', 'A549_Q3', 'Other_Q3', 'All_Q3']].rank(ascending=False).mean(axis=1)
