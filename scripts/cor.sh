...
if [ "$flag" == --merge ] || [ "$flag" == --ploidy ] || [ "$flag" == --aswp ]; then
  ...
  parallel --jobs "$njobs" python3 "$scripts_path"CORRELATIONanalysis/Annotate_SNPs_with_ploidy.py :::: "$parallel_parameters_path"ASWP_parameters.cfg
fi

python3 "$scripts_path"PARAMETERS/MakeParametersForCS.py
parallel --jobs "$njobs" python3 "$scripts_path"CORRELATIONanalysis/CorStats.py :::: "$parallel_parameters_path"CS_parameters.cfg
